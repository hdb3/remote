...
yum -y -q upgrade
PACKAGES="patch crudini mod_wsgi python-openstackclient mariadb MySQL-python ntp"
...
if [[ $MY_ROLE =~ "compute" ]] ; then
  PACKAGES="$PACKAGES openstack-nova-compute sysfsutils openstack-neutron openstack-neutron-ml2 openstack-neutron-linuxbridge ebtables"
fi
if [[ $MY_ROLE =~ "network" ]] ; then
  PACKAGES="$PACKAGES openstack-neutron openstack-neutron-ml2 openstack-neutron-linuxbridge ebtables ipset"
fi
yum -y -q install $PACKAGES
else
  echo "this script now only supports Mitaka release version...."
  exit 1
fi
